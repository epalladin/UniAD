...
class UniADTrackLidar(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        super(UniADTrackLidar, self).__init__(
        ...
        print("Load LidarBackbone weights")
        checkpoint_backbone_filename = "/mnt/efs/users/edoardo.palladin/Repos/UniAD/ckpts/r50_fpn_voxel_0075.pth"

        checkpoint = torch.load(checkpoint_backbone_filename, map_location='cpu')
        ...
        for model, name_model in zip(
            [self.pts_voxel_layer, self.pts_voxel_encoder, self.pts_middle_encoder, self.pts_backbone, self.pts_neck], 
            ["pts_voxel_layer.", "pts_voxel_encoder.", "pts_middle_encoder.", "pts_backbone.", "pts_neck."]):
            print(name_model)
            param_to_load =  [name_model + name for name, _ in model.named_parameters()]
            new_state_dict = OrderedDict({k.replace(name_model, ""): v for k, v in state_dict.items() if k in param_to_load})
            load_state_dict(model, new_state_dict, strict=False)
            # load_checkpoint(model, checkpoint_backbone_filename, map_location='cpu')
        print("Loaded LidarBackbone weights")

    def extract_pts_feat(self, pts, img_metas, prev_bev=None):
    ...
    def get_history_bev(self, points_queue, img_metas_list):
        """
        Get history BEV features iteratively. To save GPU memory, gradients are not calculated.
        """
        self.eval()
        len_queue = len(points_queue)
        with torch.no_grad():
            prev_bev = None
            for i in range(len_queue):
                points = points_queue[i]
                img_metas = [each[i] for each in img_metas_list]
                prev_bev = self.extract_pts_feat(pts=points, img_metas=img_metas, prev_bev=prev_bev)[0]
        self.train()
        return prev_bev

    # Generate bev using bev_encoder in BEVFormer
    def get_bevs(self, points, img_metas, prev_points=None, prev_img_metas=None, prev_bev=None):
        if prev_points is not None and prev_img_metas is not None:
            assert prev_bev is None
            prev_bev = self.get_history_bev(prev_points, prev_img_metas)

        if self.freeze_bev_encoder:
            with torch.no_grad():
                points_feats = self.extract_pts_feat(pts=points, img_metas=img_metas, prev_bev=prev_bev)[0]
        else:
                points_feats = self.extract_pts_feat(pts=points, img_metas=img_metas, prev_bev=prev_bev)[0]

        bev_embed = points_feats.view(points_feats.shape[0], points_feats.shape[1], -1).permute(2, 0, 1)  # torch.Size([40000, 1, 256])
        assert bev_embed.shape[0] == self.bev_h * self.bev_w

        bev_mask = torch.zeros((points_feats.shape[0], self.bev_h, self.bev_w), device=bev_embed.device, dtype=bev_embed.dtype)
        bev_pos = self.pts_bbox_head.positional_encoding(bev_mask) #  torch.Size([1, 256, 200, 200])

        return bev_embed, bev_pos # torch.Size([40000, 1, 256]), torch.Size([1, 256, 200, 200])

    @auto_fp16(apply_to=("points", "prev_bev"))
    def _forward_single_frame_train(
        ...
        Args:
            points: list[torch.Tensor] shape [P, 4]
        ...
        bev_embed, bev_pos = self.get_bevs(
            points, img_metas,
            prev_points=prev_points, prev_img_metas=prev_img_metas,
        )
    ...
    def forward_track_train(self,
        ...
        for i in range(num_frame):
            ...
            boxes = gt_bboxes_3d[0][i].tensor.to(points[0].device)
            # normalize gt bboxes here!
            boxes = normalize_bbox(boxes, self.pc_range)
            sd_boxes = gt_sdc_bbox[0][i].tensor.to(points[0].device)
        ...
        for i in range(num_frame):
            # prev_points = points[:, :i, ...] if i != 0 else points[:, :1, ...]
            prev_points = points[:i] if i != 0 else points[:1]
            prev_img_metas = copy.deepcopy(img_metas)
            # TODO: Generate prev_bev in an RNN way.

            points_single = points[i] # torch.stack([point_[i] for point_ in points], dim=0)
    ...
    def _forward_single_frame_inference(
        ...
        # NOTE: You can replace BEVFormer with other BEV encoder and provide bev_embed here
        bev_embed, bev_pos = self.get_bevs(points, img_metas, prev_bev=prev_bev)
        det_output = self.pts_bbox_head.get_detections(
            bev_embed, 
            object_query_embeds=track_instances.query,
            ref_points=track_instances.ref_pts,
